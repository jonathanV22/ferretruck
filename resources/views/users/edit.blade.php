        ...
        <div class="grid bg-white rounded-lg shadow-xl w-11/12 md:w-9/12 lg:w-1/2">
          ...
          <form method="POST" action="{{url('/users/'.$user->id)}}" enctype="multipart/form-data">
            ...
            @csrf
              ...
              <div class="grid grid-cols-1 mt-5 mx-7">
              <label class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold mb-1">Subir foto
                {{$user->foto}}
              </label>
                  <div class='flex items-center justify-center w-full'>
                      <label class='flex flex-col border-4 border-dashed w-full h-32 hover:bg-gray-100 hover:border-blue-300 group'>
                          <div class='flex flex-col items-center justify-center pt-7'>
                            @if (isset($user->foto))
                            <div>
                              <img src="{{ asset('storage').'/'.$user->foto}}" alt="foto perfil" class="w-20 h-20 mr-8 ml-8 object-center" name="foto">
                            </div>  
                            @else
                      ...
                      <input type='file' class="hidden" name="foto" value="{{$user->foto}}"/>
                      </label>
                  </div>
              </div>  
